...
Transcribes individual audio segments using Whisper
"""
...
try:
    ...
    print("pip install faster-whisper torch torchaudio")
    print(f"Missing: {e}")
    exit(1)
...
class SegmentTranscriber:
    ...
    def _determine_device(self, device: str) -> str:
        ...
        if device == "auto":
            if torch.cuda.is_available():
                ...
                # faster-whisper doesn't support MPS directly, but we can try CPU with MPS fallback
                logger.info("MPS available but faster-whisper doesn't support it directly. Using CPU.")
                return "cpu"
            else:
                return "cpu"
        return device
    ...
    def _transcribe_segment(self, audio_file: str) -> Dict[str, Any]:
        ...
        try:
            # Transcribe the audio segment with faster-whisper
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Audio Segment Transcription Tool")
    ...
    parser.add_argument("-d", "--device", default="auto",
                       help="Device to use (auto, cpu, cuda). Note: MPS not supported by faster-whisper")
    ...
    if not args.output:
        ...
        args.output = input_path.with_name(f"{input_path.stem}_transcribed.json")
